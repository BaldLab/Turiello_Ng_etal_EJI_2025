...
dev.copy(pdf, paste0(output.dir, "Figure_X_wnnUMAP_DensityPlot_NKG7.pdf"))
dev.off()
...
dev.copy(pdf, paste0(output.dir, "Figure_X_VlnPlot_NKG7_ordered.pdf"))
dev.off()
...
cyto.genes <- cytotoxicity.markers

# Extract expression matrix
expr <- GetAssayData(pbmc.seurat, layer = "data", assay = "alra")  

# Initialize result dataframe
cyto.stats <- lapply(cyto.genes, function(gene){
  vals <- expr[gene, ]
  data.frame(
    Gene              = gene,
    Percent_Positive  = mean(vals > 0) * 100,
    Mean_Expression   = mean(vals),
    Mean_Pos_Only     = ifelse(sum(vals > 0) > 0, mean(vals[vals > 0]), NA),
    Median_Pos_Only   = ifelse(sum(vals > 0) > 0, median(vals[vals > 0]), NA),
    Max_Expression    = max(vals)
  )
}) %>% bind_rows()

# Round all numeric columns to 2 significant figures
cyto.stats_rounded <- cyto.stats %>%
  mutate(across(where(is.numeric), ~ signif(., digits = 4)))

write.csv(cyto.stats_rounded, paste0(output.dir, "cytotoxicity_gene_stats_pbmc_dataset_alra.csv"))


```

### Figure 4 - Co-expression boolean upset plots without top 10 limitation
```{r Figure_4_UpsetPlots_expanded}
...
# Set SCT assay and identity
DefaultAssay(pbmc.seurat) <- "SCT"
Idents(pbmc.seurat) <- pbmc.seurat@meta.data$custom_anno

# Define cytotoxicity marker of interest
marker <- "NKG7"  

# Define populations to analyze
subset.var <- c("CD8_EM", "CD4_CTL", "NK_Bright", "NK_Dim")

# Initialize results list
nkg7.freq.df <- data.frame(
  Population      = character(),
  Total_Cells     = integer(),
  NKG7_Positive   = integer(),
  NKG7_Negative   = integer(),
  Percent_NKG7_Negative = numeric(),
...
for(j in seq_along(subset.var)){
  ...
  pop <- subset.var[j]
  message("Analyzing population: ", pop)
  
  # Subset the dataset
  temp.seurat <- subset(pbmc.seurat, idents = pop)
  
  # Extract NKG7 expression
  expr.vec <- FetchData(temp.seurat, vars = marker)[,1]
  
  # Determine which cells are NKG7-negative
  nkg7.negative <- expr.vec <= theshold.val
  
  # Summarize
  total <- length(expr.vec)
  n.neg <- sum(nkg7.negative)
  n.pos <- total - n.neg
  percent.neg <- round(100 * n.neg / total, 2)
  
  # Add to results table
  nkg7.freq.df <- rbind(nkg7.freq.df, data.frame(
    Population = pop,
    Total_Cells = total,
    NKG7_Positive = n.pos,
    NKG7_Negative = n.neg,
    Percent_NKG7_Negative = percent.neg
  ))
}
...
VlnPlot_scCustom(pbmc.seurat, "GZMB", pt.size = 0)
...
for(j in seq_along(subset.var)){
  ...
  dev.copy(pdf, paste0(output.dir, "Upset_plot_", subset.var[j], "_top20_intersects.pdf"))
  dev.off()
